...
**Leyu 是一个面向后台管理系统的轻量级基础模板，基于 Spring Boot + MyBatis-Plus 构建。**

它不是只停留在 CRUD 的脚手架，而是把后台项目里最常见、也最容易失控的几件事先统一起来：认证鉴权、权限建模、数据权限、多租户、代码生成、接口文档，以及前后端之间的响应与枚举口径。

核心优势包括：

- 统一响应结构与前端枚举下发，减少前后端口径不一致
- JWT 登录鉴权
- 深度集成 Swagger / OpenAPI，接口文档与响应结构开箱即用
- 完整的 RBAC 权限模型，支持全局、模块、接口级权限控制
- 基于角色的数据权限控制，支持全部、本部门、本部门及以下、仅本人
- 权限代码自动生成，降低手工维护成本
- 表驱动代码生成，可按模板生成 controller、entity、query、dto、vo 等代码
- 内置多租户支持，便于扩展为团队级后台基线
...
### UI 界面
访问主站：  
[http://leyu.xusheng94.com](http://leyu.xusheng94.com)

### API 文档 (Swagger)
访问 Swagger UI：  
[http://leyu.xusheng94.com/swagger-ui/index.html](http://leyu.xusheng94.com/swagger-ui/index.html)
...
### 4️⃣ 数据权限基于角色 dataScope 自动拼装

- 角色表 `sys__role` 提供 `data_scope` 字段，用于声明当前角色的数据范围
- 当前已支持的数据范围：
   - `ALL`：全部数据
   - `DEPT_AND_CHILD`：本部门及以下数据
   - `DEPT`：本部门数据
   - `SELF`：仅本人数据
- 数据权限通过 MyBatis-Plus 数据权限插件统一拼装到 SQL 中，不依赖前端传参
- 仅当目标表实际存在 `dept_id` / `creator_id` 字段时，才会自动附加数据权限条件
- 平台用户默认跳过数据权限限制

当前实现约定：

- `dept_id`：用于部门范围过滤
- `creator_id`：用于“仅本人数据”过滤
- `sys__dept.path`：用于 `DEPT_AND_CHILD` 的部门子树匹配

这意味着业务表只要遵循上述字段约定，就可以复用统一的数据权限逻辑。
