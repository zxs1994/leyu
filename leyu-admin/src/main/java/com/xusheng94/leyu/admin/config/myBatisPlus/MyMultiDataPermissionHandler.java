...
public class MyMultiDataPermissionHandler implements MultiDataPermissionHandler {
  ...
  public Expression getSqlSegment(Table table, Expression where, String mappedStatementId) {
    ...
    Set<String> columns = loadTableColumns(tableName);
    boolean hasCreatorId = columns.contains(CREATOR_ID_COLUMN);
    boolean hasDeptId = columns.contains(DEPT_ID_COLUMN);
  ...
  private Set<String> loadTableColumns(String tableName) {
    ...
    String cacheKey = REQUEST_CACHE_PREFIX + "tableColumns:" + normalizedTableName;
    // 同一个 HTTP 请求里，同一张表的字段结构不会变化，避免重复查 information_schema。
    Set<String> cachedColumns = getStringSetRequestCache(cacheKey);
    ...
    Set<String> columns = new HashSet<>(jdbcTemplate.query(
        ...
        normalizedTableName));
    putRequestCache(cacheKey, columns);
    return columns;
  }
  ...
  private Set<String> getStringSetRequestCache(String key) {
    Object value = getRequestCache(key);
    if (value instanceof Set<?> set && set.stream().allMatch(String.class::isInstance)) {
      return set.stream().map(String.class::cast).collect(java.util.stream.Collectors.toSet());
    }
    return null;
  }
